...
plot_predictions <- function(model, new_data) {
    ...
    suppressWarnings({
        g <- model %>% predict(new_data) %>% bind_cols(new_data %>% 
             ...
             mutate(key = fct_rev(key)) %>% mutate(label_text = str_glue("Price: {scales::dollar(value)} ({key})
                                                                        Model : {Model}
                                                                        Category: {Category}
            ...
            expand_limits(y = 0) + theme_tq() + scale_color_tq() + coord_flip() + labs(title = "Prediction vs Actual")})
...
plot_price_vs_weight <- function() {
    ...
    g <- price_vs_weight_tbl %>% mutate(label_text = str_glue("Model: {Model}
        ...
        ggplot(aes(Weight_lb, Price_num, color = Category)) + geom_point(aes(text = label_text)) + geom_smooth(span = 3, se = FALSE) +
        scale_color_tq() + scale_y_continuous(labels = scales::dollar_format()) + theme_tq() +
        labs(title = "2019 Cannondale Bicycles: Understanding the Pricing Model", y = "Price (USD)", x = "Weight (lbs)")
    
    plotly::ggplotly(g, tooltip = "text") %>% layout(legend = list(x = 0.8, y = 0.9))}
```

# Modeling Using parsnip

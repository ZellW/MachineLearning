...
# FORECASTING RESULTS
# METHOD: GLMNET - FLAGGED ANOMALIES
...
forecast_flag_anomalies_tbl <- pages_most_visited_anom_tbl %>%
    select(Page, Date, observed, anomaly, visits_cleaned) %>%
    nest() %>%
    ...
    mutate(model = map(split, .f = function(split) {
        ...
        model <- linear_reg(mixture = 0.5) %>%
            set_engine("glmnet") %>%
            fit.model_spec(observed ~ . - Date - visits_cleaned, data = trn_tbl)
    ...
    mutate(actual_vs_preds = map2(.x = split, .y = predictions, .f = function(split, predictions) {
...
forecast_plot_flagged_anoms_vs_observed <- forecast_flag_anomalies_tbl %>%
    ...
    labs(title = "Forecast Plot - Flagged Anomalies")

# 2.2 Cleaned Vs Pred -----
forecast_plot_flagged_anoms_vs_cleaned <- forecast_flag_anomalies_tbl %>%
    select(Page, actual_vs_preds) %>%
    unnest() %>%
    ...
    expand_limits(y = 0) +
    scale_y_continuous(labels = scales::comma) +
    labs(title = "Forecast Plot - Flagged Anomalies - Anomalies Removed")

# 3.0 Calculate MAPE ----
mape_flagged_anoms <- forecast_flag_anomalies_tbl %>%
    select(Page, actual_vs_preds) %>%
    unnest() %>%

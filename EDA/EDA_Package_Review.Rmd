...
The `ExPanDaR` package offers a toolbox for interactive exploratory data analysis (EDA).

The new `ExPanDaR` version contains an option to export data and code as a notebook so that you can take your exploratory data analysis to the next level. All it takes is one additional parameter so that you do not accidentally share your data with the world in case you are publicly hosting your shiny app:
...
Explore a Shiny version [here](https://jgassen.shinyapps.io/expand_fuel_economy/)

## Overall Impression

A quick way to review data.  Worth keeping in the EDA toolbelt.

```{r echo=FALSE}
detach("package:ExPanDaR", unload=TRUE)
```
...
Easily does 3-way tables:

```{r}
starwars %>% filter(species == "Human") %>% tabyl(eye_color, skin_color, gender)
```

Simple to add to the tables:
...
starwars %>% filter(species == "Human") %>% tabyl(eye_color, skin_color, gender) %>% 
     ...
     adorn_title("combined")
```
Learn more [here](https://github.com/sfirke/janitor/blob/master/vignettes/tabyls.md).

Much more to learn including:

- `remove_empty()` rows and columns (really helpful when importing from Excel)
- `remove_constant()` columns
- Fix dates stored as serial numbers with `excel_numeric_to_date()` - awesome!
- Elevate column names stored in a data.frame row using `rows_to_names`

## Overall Impression

This is an easy one - simply use it.
...
myIDs <- filter(df_status(dataset), grepl("id", variable, ignore.case = TRUE)) %>%
     select(variable, type, unique)
myIDs[,1]
...
temp <- dataset[colnames(dataset[, sapply(dataset, is.character)])] %>%
     summarise_all(funs(n_distinct(.)))

factor_columns <- melt(as.data.frame(temp)) %>% filter(value <= 15) %>% 
     select(variable) %>% .[["variable"]] %>% as.character()
...
```{r}
myData <- funModeling::heart_disease

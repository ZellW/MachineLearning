...
prp(
  temp, extra = 101, box.palette = "#f9f5f1", type = 4, cex = 0.7, yspace = 4, nn = TRUE, nn.cex = 1.4,
...
Although we built a recursive partitioning model earlier to select the modelling variables, we’ll run it again here inside the caret package’s train function. We’ll do that primarily to tune / refine the model, and partly to create consistent outputs across our five models. Consistency, for example, will enable us to pluck some of the information out of the final models to annotate the concluding plot.
...
Random Forest broadly combines a large number of decision trees, by deploying a bagging procedure to repeatedly split the training data horizontally, and a further procedure to randomly sample the predictors vertically.
...
At first blush, we might be tempted to model with an mtry of 2 given the four variables of borough, major category, year and month. However, our categorical variables will actually generate 54 dummy variable, so an mtry of 54 / 3 = 18 would be more appropriate. We can see the impact of this choice below.
...
pred_meas <- map_df(list(model_GLM, model_RF, model_SGB, model_RP, model_Cub), function(x) {
  tibble(model = x$modelInfo$label, method = x$method, model_type = x$modelType, rmse = rmse(x, crime_df), rsquare = rsquare(x, crime_df))
})
```
...
ggplot(combined_pred, aes(pred, act, colour = maj_cat)) + geom_point(alpha = 0.3, size = 2) +
     ...
     scale_colour_economist(name = "Major Category") + scale_y_continuous(breaks = seq(500, 3000, by = 500),
                                                                          limits = c(0, 3000)) +
